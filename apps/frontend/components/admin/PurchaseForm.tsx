...
const getFieldClasses = (hasError?: boolean, extra?: string) => {
  const base = `mt-1 sm:text-sm w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-rose-500 focus:border-rose-500 ${
...
export const PurchaseForm: React.FC<PurchaseFormProps> = ({
  ...
  useEffect(() => {
    if (isEditMode) {
      ...
      if (currentItems && currentItems.length > 0) {
        const existingCartItems: CartItem[] = currentItems.map(item => {
        ...
        setTimeout(() => setCartItems(existingCartItems), 0);
      }
    }
  }, [isEditMode, watch('items'), products]);
  ...
  useEffect(() => {
    if (selectedProduct && !purchasePrice) {
      ...
      setTimeout(() => setPurchasePrice(defaultPrice.toString()), 0);
    }
          ...
          >
            {isLoading ? (
              ...
              <>
                ...
                {isEditMode ? 'Actualizar Compra' : 'Guardar Compra'} ({cartItems.length} productos)
              </>
            )}
          </button>
        </div>
